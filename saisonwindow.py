...
from saison import Ui_SaisonWindow
from saisoncontroller import SaisonController
#from menuhandler import MenuHandler
from PyQt5 import QtWidgets, QtCore
import logging

class SaisonWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.ui = Ui_SaisonWindow()
        self.ui.setupUi(self)

        self.db_name = "werder.db"
        self.controller = SaisonController(self.db_name)

        self.ui.saveBtn.clicked.connect(self.save_saison)

        self.refresh_table()

        # Verbinden Sie das keyPressEvent-Ereignis des TableWidgets
        self.ui.saisontbl.keyPressEvent = self.custom_keyPressEvent
    ...
    def save_saison(self):
        new_rows = []
        existing_data = self.controller.get_all_saison()
        existing_ids = {row[0] for row in existing_data}  # Menge der vorhandenen IDs

        for row in range(self.ui.saisontbl.rowCount()):
            row_data = []
            for column in range(self.ui.saisontbl.columnCount()):
                item = self.ui.saisontbl.item(row, column)
        ...
        self.controller.delete_saison()

        if new_rows:
            # Hier werden die neuen Daten validiert und gespeichert
            self.controller.save_saison(new_rows)
    ...
    def refresh_table(self):
        saison_data = self.controller.get_all_saison()

        self.ui.saisontbl.setRowCount(len(saison_data) + 1)  # Eine zusätzliche Zeile hinzufügen
        self.ui.saisontbl.setColumnCount(10)  # Setzen Sie die Anzahl der Spalten entsprechend Ihrer Datenbanktabelle

        for row_index, row_data in enumerate(saison_data):
            for column_index, column_data in enumerate(row_data):
                item = QTableWidgetItem(str(column_data))
                self.ui.saisontbl.setItem(row_index, column_index, item)

        #Entfernen der Zeilennummerierung
        self.ui.saisontbl.verticalHeader().setVisible(False)
    ...
    def custom_keyPressEvent(self, event):
        if event.key() == QtCore.Qt.Key_Tab:
            current_row = self.ui.saisontbl.currentRow()
            current_col = self.ui.saisontbl.currentColumn()
            if current_col == self.ui.saisontbl.columnCount() - 1 and current_row == self.ui.saisontbl.rowCount() - 1:
                # Fügen Sie eine neue Zeile hinzu, wenn Tab in der letzten Spalte und letzten Zeile gedrückt wird
                self.ui.saisontbl.insertRow(self.ui.saisontbl.rowCount())
                # Setzen Sie den Fokus auf die erste Zelle der neuen Zeile
                self.ui.saisontbl.setCurrentCell(current_row + 1, 0)
            else:
                # Weiterleitung des keyPressEvent-Ereignisses an die Basisimplementierung
                QtWidgets.QTableWidget.keyPressEvent(self.ui.saisontbl, event)
        elif event.key() == QtCore.Qt.Key_Delete:  # Hinzufügen für Entf-Taste
            selected_rows = self.ui.saisontbl.selectedItems()
            if selected_rows:
                rows_to_delete = set(item.row() for item in selected_rows)
                for row in sorted(rows_to_delete, reverse=True):
                    saison_id = self.ui.saisontbl.item(row, 0).text()  # Annahme: ID ist in der ersten Spalte
                    self.controller.delete_saison()
                    self.ui.saisontbl.removeRow(row)
        else:
            # Weiterleitung des keyPressEvent-Ereignisses an die Basisimplementierung
            QtWidgets.QTableWidget.keyPressEvent(self.ui.saisontbl, event)
